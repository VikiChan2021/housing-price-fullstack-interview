...
当前测试均为计划状态。实现时将测试文件或测试用例 ID 填入“证据”列。

| 需求 | 责任组件 | 主要验收 | 计划证据 |
|---|---|---|---|
| SYS-001~003 | `apps/web`、三个服务 | AC-401、AC-503 | Portal E2E + 服务调用日志 |
| ML-001 | `services/ml-api` | AC-101、AC-102 | 训练单元测试 + 指标文件 |
| ML-002 | `services/ml-api` | AC-103~105 | API 契约测试 |
| ML-003 | `services/ml-api` | AC-106 | `/model-info` 响应快照 |
| ML-004 | `services/ml-api` | AC-107 | 健康/损坏模型测试 |
| ML-005~006 | `services/ml-api`、Docker | AC-002、AC-108 | Docker 构建 + Swagger 截图 |
| EST-001~003 | `apps/web`、`estimator-api` | AC-201~203 | 组件测试 + Estimator E2E |
| EST-004~005 | `apps/web` | AC-204~205 | localStorage/比较 E2E |
| EST-006~007 | `estimator-api` | AC-206 | 集成测试 + 下游故障测试 |
